...
5. **Confirm the run** by passing the complete quote evidence unchanged to the confirmation tool, exactly as Prepare returned it. Keep `quote_token` in tool calls only; never show it in ordinary prose. The run charges at most `estimated_max_credits`. Changing a quantity or selecting **Update quote** authorizes only a fresh quote, never a paid run. Confirm only the current quote after the customer explicitly confirms it or uses its **Confirm run** action.
...
- **Do not audit jobs via `enrichley_get_job_metadata`.** That tool renders a widget rather than text, so the purpose of a job is not recoverable from it. To audit existing discoveries and understand what was run, use `enrichley_list_company_lookalike_discoveries` and `enrichley_list_people_search_discoveries` instead - page through them fully rather than reading only the first page.
...
- **Quotes go stale.** If time passed since the quote, prepare a fresh one rather than confirming an old token.
...
- A customer may confirm a paid run directly from the quote card; that confirmation never appears in your context. If the customer says they confirmed from the card, trust them: check the jobs list or use the run id shown on their card instead of asserting that no run exists.
- Failed or partial runs settle automatically - unused reserved credits are released. If numbers look off, re-check job status before concluding anything. The actual cost appears as `settled_credits` on terminal job status; it is absent while a run is active or settlement is still pending.
